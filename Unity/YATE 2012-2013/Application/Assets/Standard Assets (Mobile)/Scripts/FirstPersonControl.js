...
public var currentSound: AudioClip;
...
function Update()
{
	...
	if (Mathf.Abs(moveTouchPad.position.x)>0 || Mathf.Abs(moveTouchPad.position.y)>0) {
		if (!audio.isPlaying && character.isGrounded && movement!=Vector3(0,0,0)) {
			var n = System.Math.Round(Random.Range(0,4));
	    	currentSound = walkSounds[n];//walkSounds[Random.Range(0, walkSounds.length)];
